...
function configure (app, wares, ctx, env) {
    ...
    ctx.alexa.configureIntentHandler('AddTreatment', function (callback, slots, sbx) {
        ...
        $.ajax({ url: 'https://lennoxt1d.azurewebsites.net/api/v1/treatments', method: 'POST', data: treatment, headers: { 'api-secret': '5d751d21f555acd9a668d2c1150365427251e043' }
        }).done( function(data) {
            callback('Successfully addded treatment', 'Successfully addded treatment');
        }).fail( function(jqXHR, textStatus, errorThrown) {
            callback('treatment not added', textStatus);
        });

        // api.post('/treatments/', , function (req, res) {
        //   ctx.treatments.create(treatment, function (err, created) {
        //     if (err) {
        //       callback('[Test] treatment not added', err);
        //     } else {
        //       callback('[Test] Successfully added treatment', 'success');
        //     }
        //   });
        // });
    });
