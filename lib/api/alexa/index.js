...
function configure (app, wares, ctx, env) {
    ...
    api.use(ctx.authorization.isPermitted('api:treatments:create'));
    ...
    ctx.alexa.configureIntentHandler('AddTreatment', function (callback, slots, sbx) {
        var glucose = slots.glucose.value || nil,
            carbs = parseFloat(slots.carbs.value) || nil,
            reason = parseFloat(slots.reason.value) || nil,
            insulin = parseFloat(slots.insulin.value) || nil,
            enteredBy = slots.enteredBy.value || 'Mom';
        var treatment = [{
        ...
        treatments.create(treatment, function (err, created) {
            if (err) {
                callback('[Test] treatment not added', err);
            } else {
                // var status = 'New treatment added with glucose: ' + slots.glucose.value + ', carbs: ' + slots.carbs.value + ' and insulin: '+ slots.insulin.value;
                callback('[Test] Successfully added treatment', created);
            }
        });
    });
