...
function configure (app, wares, ctx, env) {
    ...
    ctx.alexa.configureIntentHandler('AddTreatment', function (callback, slots, sbx) {
        ...
        var treatment = [{ "eventType": "alexa", "enteredBy": "viral"}];

        api.post('/treatments/', wares.bodyParser({limit: 1048576 * 50 }), ctx.authorization.isPermitted('api:treatments:create', function (req, res) {
          treatments.create(treatment, function (err, created) {
            if (err) {
              callback('[Test] treatment not added', 'Error');
            } else {
